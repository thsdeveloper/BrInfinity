            ...
            @foreach($producao as $key => $items)
                <tr>
                  ...
                  @foreach($items as $keyitems => $value)
                    <td>
                      <button type="button" class="btn-intermediations btn btn-link btn-xs" data-toggle="modal" data-target="#myModal" intermediations_id="{{$value[1]}}">@if($value[1] > 0){{$value[0]}} @else -- @endif</button>
                    </td>
